                ...
                while float(value) > 1:
                    ...
                    for line in f.readlines():
                        ...
                        if float(value) > 1: # this means there is overlap, therefore move inlet further downstream
                            ...
                            #PB no clue why multiply with subcatchment???
                            pcrasterCommand(pcrcalc + " 'F0 = upstream(F1,scalar(F2))'",
                                            {"F0": tmp3_map, "F1": ldd_map,
                                             "F2": tmp2_map})  # move directly connected stations 1 pixel downstream
