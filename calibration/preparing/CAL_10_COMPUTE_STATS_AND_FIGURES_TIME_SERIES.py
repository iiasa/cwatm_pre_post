...
for index, row in stationdata_sorted.iterrows():
    ...
    Val_Start = datetime.datetime.strptime(row['Val_Start'], "%d/%m/%Y")
    ...
    if row['Val_Start'][:10] != "Streamflow":  # Check if Q record is long enough for validation
        Val_Start = datetime.datetime.strptime(row['Val_Start'], "%d/%m/%Y")
        Val_End = datetime.datetime.strptime(row['Val_End'], "%d/%m/%Y")
    ...
    Cal_Start = datetime.datetime.strptime(row['Cal_Start'], "%d/%m/%Y")
    Cal_End = datetime.datetime.strptime(row['Cal_End'], "%d/%m/%Y")
    ...
    if os.path.isfile(os.path.join(path_subcatch, "streamflow_simulated_best.tss")):
        print ("Making figures for catchment " + row['ID'] + ", size " + str(row['CatchmentArea']) + " pixels...")
        Qsim = pandas.read_csv(os.path.join(path_subcatch, "streamflow_simulated_best.tss"), sep=",", parse_dates=True, index_col=0, header=None)
    else:
        print ("streamflow_simulated_best.csv missing for catchment" + row['ID'] + "...")
        continue

    Qsim = Qsim.loc[:, 1]  # from dataframe to series
    ...
    if row['Val_Start'][:10] != "Streamflow":  # Check if Q record is long enough for validation
        ...
        Q_sim_Val = Q.loc[Val_Start:Val_End].iloc[:, 0].values
        Q_obs_Val = Q.loc[Val_Start:Val_End].iloc[:, 1].values
    ...
    fig.savefig(os.path.join(SubCatchmentPath, "FIGURES", row['ID'] + '_summary.png'), dpi=300, format='PNG', bbox_inches='tight')
    plt.close("all")

stationdata_sorted.to_csv(Qgis_out, ',')
